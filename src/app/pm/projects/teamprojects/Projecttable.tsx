...
export default function Projecttable() {
  ...
  const getList = async () => {
    setLoading(true)
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/projects/teamprojectlistmanager?searchproject=${search}&page=0&limit=10`,{
      withCredentials: true,
      headers: {
        'Content-Type': 'application/json'
        }
      ...
      <div className=' w-full flex items-center justify-between'>
        ...
        <TableBody>
          {list.map((item, index) => (
            ...
            <TableCell className="">
              <a href={`/pm/graph/jobcomponent?teamid=${item._id}`} className=' w-fit bg-red-700 rounded-sm p-1 text-white flex items-center gap-2'>Job Component<Eye size={15}/></a>
              </TableCell>
