  ...
  useEffect(() => {
    ...
    try {
      const timer = setTimeout(() => {
        const getList = async () => {
          const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/wfh/listwfhrequestmanager?statusfilter=Pending&fullnamefilter&page=0&limit=10`,{
      ...
      const fetchLeaveData = async () => {
        ...
        try {
          const response = await axios.get(
            `${process.env.NEXT_PUBLIC_API_URL}/wfh/listwfhrequestmanager?page=${currentpage}&limit=10&statusfilter=${status}&fullnamefilter=${searchName}`,
            {
              withCredentials: true,
              headers: {
                'Content-Type': 'application/json',
