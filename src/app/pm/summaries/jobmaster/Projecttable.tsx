                  ...
                  project.jobComponents.map((job, index) => (
                    <TableRow key={index} className={``}>
                      <TableCell className={` ${clientColor(project.priority)} underline cursor-pointer`}>
                        <a target='_blank' href={`/pm/graph/jobcomponent?teamid=${project.teamid}&jobno=${job.id}&teamname=${project.teamname}`} className={`${job.isVariation ? 'text-red-500' : 'text-black'}`}>{project.jobno}</a>
                      ...
                      <TableCell className={` flex items-center gap-2 h-[50px] ${clientColor(project.priority)} ${job.isVariation ? 'text-red-500' : 'text-black'}`}>{job.members?.map((item, index) => (
                        <a href={`/pm/individualworkload?employeeid=${item.id}&name=${item.initial}&teamname=${job.name}`} className=' underline cursor-pointer'>{item.initial || ''}</a>
