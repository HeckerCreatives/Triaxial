...
export default function Teamstable() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const getList = async () => {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/teams/listteammanager?teamnamefilter=${searchteam}&page=${currentpage}&limit=10`,{
          withCredentials: true,
          headers: {
            'Content-Type': 'application/json'
            }
      ...
      <div className=' w-full flex flex-col max-w-[1520px]'>
        ...
        <TableBody>
          {teamlist.map((item, index) => (
            ...
            <TableCell>
                <Viewbtn disabled={false} name='View' onClick={() => router.push('/pm/team/teamproject')}/>
            </TableCell>
            <TableCell>
