   ...
   <div className=' w-full h-full flex flex-col bg-secondary p-4 text-zinc-100'>
    ...
    <div className=' flex flex-col gap-2 text-xs bg-primary p-4 w-fit'>
      ...
      <div className=' flex items-center gap-4'>
        ...
        <button onClick={() => router.push(`/hr/graph/individualworkload?employeeid=${list[0].teamleader.employeeid}`)} className=' ml-4 text-[0.5rem] bg-red-600 p-1 rounded-sm text-white'>View Workload</button>
      </div>
    </div>
        ...
        <TableBody>
          {list[0]?.members.map((item, index) => (
            <TableRow key={index}>
              ...
              <TableCell className="">
                <button onClick={() => router.push(`/hr/graph/individualworkload?employeeid=${item.employeeid}`)} className=' text- xs bg-red-600 p-2 rounded-sm text-white'>View Workload</button>
              </TableCell>
