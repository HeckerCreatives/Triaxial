            ...
            {list.map((graphItem, graphIndex) =>
              graphItem.members.map((member, memberIndex) => (
                <tr key={`${graphIndex}-${memberIndex}`} className="bg-primary text-[.6rem] py-2 h-[30px] border-[1px] border-zinc-600 border-collapse">
                  ...
                  <td onClick={() => router.push(`/superadmin/individualworkload?employeeid=${member.id}&name=${member.name}&teamname=${graphItem.name}`)} className=" border-[1px] border-zinc-600 whitespace-normal break-all text-center cursor-pointer underline text-white-400">{member.name}</td>
