        ...
        <TableBody>
            {list.map((item, index) => (
                ...
                <TableCell className={` flex items-center gap-2 ${clientColor(item.priority)} text-black `}>
                    <button onClick={() => router.push(`/superadmin/summaries/client/clienttotalinvoice?clientid=${item.clientid}`)} className=' bg-red-600 px-2 py-1 text-xs rounded-sm text-white flex items-center gap-1'><Eye size={12}/>Total Invoice</button>
                </TableCell>
                </TableRow>
            ))}
