        ...
        <TableBody>
            {list.map((item, index) => (
                ...
                <TableCell className="font-medium underline text-red-500 cursor-pointer">
                  <a href={`/superadmin/graph/jobcomponent?teamid=${item.teamId}`}>{item.teamName}</a>
                  </TableCell>
                ...
                <TableCell className=" flex items-center gap-2">
                    <button onClick={() => router.push(`/superadmin/summaries/team/teamtotalinvoice?teamid=${item.teamId}`)} className=' bg-red-600 px-2 py-1 text-xs rounded-sm text-white flex items-center gap-1'><Eye size={12}/>View</button>
                </TableCell>
                </TableRow>
            ))}
