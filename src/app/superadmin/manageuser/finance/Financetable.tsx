...
import { Employee, ProjectsSection, Teams } from '@/types/data'
...
export default function Financetable() {
  ...
  useEffect(() => {
    ...
    const timer  = setTimeout(() => {
      const getList = async () => {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/users/employeelist?positionfilter=finance&page=${currentpage}&limit=10&fullnamefilter=${searchfinance}`,{
        ...
        setFinance(response.data.data.employeelist)
  ...
  },[state, currentpage, searchfinance])

  //demote finance staff
  const promote = async () => {
      const selectedIds = selectedRows.map((row) => row.id);
      router.push('?state=true')

      if(selectedIds.length === 0){
        toast.error(`Please select a finance staff to demote`)
        setLoading(false)
      }else{
        try {
       ...
       const response = await toast.promise(request, {
        loading: 'Demoting finance staff....',
        success: `Finance staff successfully demoted to employee`,
        error: 'Error while demoting finance staff',
       });
    ...
    <div className=' w-full h-full flex justify-center bg-secondary p-6 text-zinc-100'>
      ...
      <div className=' w-full flex flex-col max-w-[1520px]'>
        <div className=' flex md:flex-row flex-col items-center justify-between gap-4'>
            ...
            <Dialog open={dialog2} onOpenChange={setDialog2}>
                ...
                <DialogContent className=' bg-secondary border-none text-zinc-100 grid grid-cols-1 lg:grid-cols-[250px,1fr]'>
                  ...
                  <div className=' flex flex-col gap-2 p-4'>
                    <DialogHeader>
                    <DialogDescription>
                      Are you sure you want to demote the selected finance staff?
                    </DialogDescription>
                    </DialogHeader>
                  <form action="" className=' flex flex-col gap-4 '>
                    <h2 className=' uppercase font-semibold text-xs'>Selected Finance Staff</h2>
                    ...
                    <div className=' w-full flex items-end justify-end gap-2 mt-8'>
                      <button onClick={promote} className=' btn-red'>Demote</button>
                    </div>
            ...
            <Dialog open={dialog3} onOpenChange={setDialog3}>
                ...
                <DialogContent className=' bg-secondary border-none text-zinc-100 grid grid-cols-1 lg:grid-cols-[250px,1fr]'>
                  ...
                  <div className=' flex flex-col gap-2 p-4'>
                    <DialogHeader>
                    <DialogDescription>
                      Are you sure you want to ban the selected finance staff?
                    </DialogDescription>
                    </DialogHeader>
                  <form action="" className=' flex flex-col gap-4 '>
                    <h2 className=' uppercase font-semibold text-xs'>Selected Finance Staff</h2>
            ...
            {/* <div className=' flex items-center gap-4'>
                <Input value={searchfinance} onChange={(e) => setSearchfinance(e.target.value)} type='text' className=' bg-primary h-[35px] text-zinc-100'/>
                <button className=' bg-red-700 px-8 py-2 rounded-sm text-xs'>Search</button>
            </div> */}

            <div className=' flex flex-col gap-1'>
                <label htmlFor="" className=' text-xs'>Search</label>
                <Input value={searchfinance} placeholder='Search hr name (clear the input to reset)' onChange={(e) => setSearchfinance(e.target.value)} type='text' className=' w-[300px] bg-primary text-zinc-100 text-xs h-[35px]'/>
