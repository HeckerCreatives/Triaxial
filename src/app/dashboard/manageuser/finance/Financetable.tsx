...
export default function Hrtable() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const getList = async () => {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/users/employeelist?positionfilter=finance&page=${currentpage}&limit=10&fullnamefilter=${searcmanager}`,{
          withCredentials: true,
          headers: {
            'Content-Type': 'application/json'
            }
  ...
  const promote = async () => {
    ...
    if(selectedIds.length === 0){
      toast.error(`Please select a finance to demote`)
      setLoading(false)
    }else{
      try {
     ...
     const response = await toast.promise(request, {
         loading: 'Demoting finance....',
         success: `Finance successfully demoted to employee`,
         error: 'Error while demoting finance',
     });
  ...
  const banEmployee = async () => {
    ...
    const response = await toast.promise(request, {
        loading: 'Banning finance....',
        success: `Banned successfully`,
        error: 'Error while banning finance',
    });
            ...
            <Dialog open={dialog2} onOpenChange={setDialog2}>
                ...
                <DialogContent className=' bg-secondary border-none text-zinc-100 grid grid-cols-1 lg:grid-cols-[250px,1fr]'>
                  ...
                  <div className=' flex flex-col gap-2 p-4'>
                    <DialogHeader>
                    <DialogDescription>
                      Are you sure you want to demote the selected project finance?
                    </DialogDescription>
                    </DialogHeader>
                  <form action="" className=' flex flex-col gap-4 '>
                    <h2 className=' uppercase font-semibold text-xs'>Selected Finance</h2>
            ...
            <Dialog open={dialog3} onOpenChange={setDialog3}>
                ...
                <DialogContent className=' bg-secondary border-none text-zinc-100 grid grid-cols-1 lg:grid-cols-[250px,1fr]'>
                  ...
                  <div className=' flex flex-col gap-2 p-4'>
                    <DialogHeader>
                    <DialogDescription>
                      Are you sure you want to ban the selected finance?
                    </DialogDescription>
                    </DialogHeader>
                  <form action="" className=' flex flex-col gap-4 '>
                    <h2 className=' uppercase font-semibold text-xs'>Selected Finance</h2>
        ...
        {managers.map((item, index) => (
             ...
             <Dialog open={dialog4} onOpenChange={setDialog4}>
                ...
                <DialogContent className=' bg-secondary border-none text-zinc-100 grid grid-cols-1 lg:grid-cols-[250px,1fr]'>
                  ...
                  >
                    <p className=' p-2 uppercase text-sm font-semibold mt-8 bg-gradient-to-r from-zinc-950 to-zinc-950/10'>Edit Finance</p>
                  </div>
