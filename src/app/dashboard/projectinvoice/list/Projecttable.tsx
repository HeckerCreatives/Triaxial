...
export default function Projecttable() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const getList = async () => {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/projects/saprojectlist?searchproject=${search}&page=${currentpage}&limit=10`,{
        ...
        <TableBody>
          {list.map((item, index) => (
            ...
            <TableCell className=" flex items-center gap-2">
              <button onClick={() => router.push(`/superadmin/projectinvoice?jobcomponentid=${item._id}`)} className=' bg-red-600 p-2 text-white rounded-sm flex items-center gap-2'><Eye size={15}/>View Invoice Projection</button>
