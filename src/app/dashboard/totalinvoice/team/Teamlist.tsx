...
export default function Teamlist() {
  ...
  useEffect(() => {
    setLoading(true)
    const getList = async () => {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/teams/listallteamssa`,{
        ...
        <TableBody className=' '>
            {list.map((item, index) => (
                ...
                <TableCell className=" flex items-center gap-2">
                    <button onClick={() => router.push(`/superadmin/totalinvoice/team/teamtotalinvoice?teamid=${item.teamid}`)} className=' bg-red-600 px-2 py-1 text-xs rounded-sm text-white flex items-center gap-1'><Eye size={12}/>View</button>
                </TableCell>
                </TableRow>
            ))}
